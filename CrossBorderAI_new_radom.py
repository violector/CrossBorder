...
class CrossBorderDQNMDP:
    def __init__(self):
        ...
        # 奖励设置
        self.R_CORRECT, self.R_MISCLASS = 70, -120  # 提高误判惩罚，促使 Agent 谨慎探索
        self.R_XB, self.R_INFO = -40, -5
        self.R_eval, self.R_mitig = -3, -4  # 缓解成本略高于评估

        # 子类场景 (a-d)
        self.sub_types = ['a', 'b', 'c', 'd']

        # 动作可能导致的状态集合 (不含固定概率，由 step 动态生成)
        self.TRANSITION_MAP = {
            'a_eval': {
                'M1': ['M1', 'M2'],
                'M2': ['M2', 'M3', 'M4'],
        ...
        self.P_DECISION_SUCCESS = {'M1': 0.95, 'M2': 0.75, 'M3': 0.6, 'M4': 0.05}
    ...
    def step(self, current_state_dict, action_idx):
        ...
        if curr_name == 's0':
            if action == 'a_next':
                states = ['M1', 'M2', 'M3', 'M4']
                next_name = np.random.choice(states, p=self._get_dynamic_probs(4))
            else:
                return self._get_state_dict('s0'), -200, True

        # 2. 动态自由探索转移 (Eval/Mitig)
        elif action in ['a_eval', 'a_mitig']:
            reward = self.R_eval if action == 'a_eval' else self.R_mitig
            possible_states = self.TRANSITION_MAP[action][curr_name]
            # 动态生成本次动作的转移概率
            dynamic_p = self._get_dynamic_probs(len(possible_states))
            next_name = np.random.choice(possible_states, p=dynamic_p)
            if action == 'a_eval':
...
# ----------------- 3. DQN Agent (含模型保存/加载) -----------------
class QNetwork(nn.Module):
    def __init__(self, state_dim, action_dim):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(state_dim, 256), nn.ReLU(),  # 增加神经元处理复杂性
...
class DQNAgent:
    def __init__(self, state_dim, action_dim):
        ...
        self.epsilon_min, self.epsilon_decay = 0.01, 0.997  # 减慢衰减以增加探索
        self.batch_size, self.target_update_iter = 64, 150
        self.learn_step_counter = 0

        self.model = QNetwork(state_dim, action_dim)
        self.target_model = QNetwork(state_dim, action_dim)
        self.optimizer = optim.Adam(self.model.parameters(), lr=0.0005)  # 降低学习率稳定训练
...
def train():
    ...
    save_path = "best_agent_random_1.pth"
    rewards_history = []
    best_mean = -float('inf')
